...
31,最短摘要的生成
=================================================================================================================================
给定一段数组存储内容的分词结构，以及关键字序列，最短摘要就是包含所有关键字的长度最短的数组
比如w0,w1,w2,w3,q0,w4,w5,q1,w6,w7,w8,q0,w9,q1，其中关键字序列为q0,q1，那么这个中很显然最短摘要为q0,w9,q1，长度为3.
算法一：1.	从数组的第一个位置开始查找出一段包含所有关键词数组Q的许立，计算当前的最短长度，并更新最短摘要数组
2.	然后从第二个位置开始，重新查找包含所有关键字的数组Q的序列，同样计算出其最短长度更新最短摘要数组。
3.	依次操作，一直到遍历至目标数组的最后一个位置。
算法一方法的时间复杂度为O(N^2*M)。
算法二：算法一的方法时间复杂度比较高，是否有办法降低呢？当然可以：
第一次扫描的时候，假设需要包含所有的关键字需要w0,w1,w2,w3,q0,w4,w5,q1，那么下次扫描可以把第一个被扫描的位置挪到q0处。即q0,w4,w5,q1,这样更新最短摘要数组，并且计算最短长度，，接下来把第一个被扫描的位置继续往后面移动一格，这样包含的序列中将减少关键字q0，然后把第二个扫描位置向后移动，这样就能找到下一个包含所有关键字的序列，然后再和之前的方法一直、知道找到长度最短的包含所有关键字序列的最短的摘要。

32，编程判断两个链表是否相交
=================================================================================================================================
算法一：直观的想法，先判断第一个链表的每个节点是否在第二个链表中。如果存在则为相交，如果不存在则两个链表不相交。
算法二: 利用计算的方法，遍历第一个链表，将链表的节点地址进行hash排序，建立hash表，并针对第二个链表的每个节点的地址查询hash表，如果它在hash表中出现，那么说明第二个链表和第一个链表有共同的节点，这个方法的时间复杂度为O(Length(h1)+Length(h2))。但是它他哦你是需要附加O(Length(h1))的存储空间，以存储哈希表。
算法三：转换为判断链表是否有环的问题，由于题目已知，把第二个链表接在第一个链表的后面，假如，两个链表相交，那么接在之后很显然第二个链表会有环，而如果两链表不相交，那么第二个链表接在第一个链表之后，第二个链表无环。而判断是否有环，可以通过步长不同的指针，比如第一个步骤为d，第二个步骤为2d，如果存在环，那么**在环内必定会相遇**，**这里其实还有一个问题，是否能求出环的入口位置？！**
算法四：遍历第一个链表到最后一个节点，然后记住最后一个节点，然后遍历第二个链表，到最后一个节点时，与第一个链表的最后一个节点做比较，若相同肯定相交，若不同则不相交。


33,求队列中最大值操作问题
=================================================================================================================================
算法一：使用两个指针分别指向队列的队头和队尾，利用一个数组来存储其中的算有元素，那么取最大值的操作就是遍历此数组所有元素，在队列长度为N的条件下，时间的复杂度为O(N)。
算法二：利用最大堆原理，可以考虑利用最大堆来维护队列中的元素，队中每个元素都有指针指向它的后续元素。这样，堆就可以很快地返回最大元素的操作。同时我们也可以保证插入和删除操作，取最大值的操作是O(1)，删除和插入的操作的复杂度为O(log2 N).
算法三：由于栈的操作都在栈顶完成，很容易维护栈中的最大值，我们可以维护一个最大值的序列(link2NextMaxItem)保存对应栈顶的最大值，这样，当我们使用两个栈来模拟队列时，可以保证平均时间复杂度是线性的。

34，求二叉树节点的最大距离
=================================================================================================================================
算法一：具体见[二叉树最远距离节点](./PDF/二叉树最远距离节点.pdf "二叉树最远距离节点.pdf")
基本原理就是分为经过根节点还有没有经过根节点来处理，即利用动态规划，每个字数最远的两个点的距离为所有点的最大值，或者经过这个子树根节点的最大距离（max1+max2+2），max1表示此子树节点的左子树的最大距离，max2表示此字数的右子树的最大距离。

算法二：


35,给定二叉树的任意两种排序结果，重构这个二叉树
=================================================================================================================================
例如前序遍历结果为：a b d c e f,中序遍历结果：d b a e c f
基本原理就是先从前序中找到根节点，然后中序遍历根节点左边的即为左子树，右边为右子树，然后子树子树也可以判断根节点、左右子树。历次递归即可重构二叉树。


36，分层遍历二叉树
=================================================================================================================================
基本原理即是比如要求反问二叉树中第k层的节点，那么其实可以把它转换成分别访问“以该二叉树根节点的左右子节点为根节点的两颗子树”中层次为k-1的节点，因此，很容易用递归的函数调用来实现，算法的实现中有需要优化的部分，如对二叉树每一层的访问都需要从根节点开始，这样效率很低，我们可以使用一个数组和两个游标记录当前访问的及诶但那和当前层次的最后一个节点的下一个位置。
还有网友写了一个不需要额外空间的见面算法：[分层遍历二叉树](./PDF/分层遍历二叉树.pdf "分层遍历二叉树.pdf")
